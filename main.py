...
from src.forecast_mode.dataloader import Load_Static_Data
from src.forecast_mode.dataloader import Load_Observation
...
AOD_dataset = Load_AOD.extract_data(ufs_aod_folder, year_list, month_list, day_list,)
METEO_SP_dataset = Load_Meteo.extract_data(ufs_meteo_folder, 'surface_pressure', year_list, month_list, day_list)
METEO_T2M_dataset = Load_Meteo.extract_data(ufs_meteo_folder, 'temperature_2m', year_list, month_list, day_list)
METEO_RH_dataset = Load_Meteo.extract_data(ufs_meteo_folder, 'humidity', year_list, month_list, day_list)
METEO_EVP_dataset = Load_Meteo.extract_data(ufs_meteo_folder, 'evaporation', year_list, month_list, day_list)
METEO_WINDU_dataset = Load_Meteo.extract_data(ufs_meteo_folder, 'wind speed u', year_list, month_list, day_list)
METEO_WINDV_dataset = Load_Meteo.extract_data(ufs_meteo_folder, 'wind speed v', year_list, month_list, day_list)
METEO_PBLH_dataset = Load_Meteo.extract_data(ufs_meteo_folder, 'pblh', year_list, month_list, day_list)
METEO_PREP_dataset = Load_Meteo.extract_data(ufs_meteo_folder, 'precipitation', year_list, month_list, day_list)
CHEM_PM25_dataset = Load_Chem.extract_data(ufs_chem_folder, 'pm25', year_list, month_list, day_list)
CHEM_O3_dataset = Load_Chem.extract_data(ufs_chem_folder, 'o3', year_list, month_list, day_list)
print('finish part 2')
print(f"Step 2 done, elapsed: {time.time() - start:.2f}s")

'''
2-2) regrid UFS-AQM data
'''
...
'''
3) Load static data, no need to regrid
'''
time_length = final_chem_pm25.dims['time']

final_static_elevation = Load_Static_Data.extract_elevation(static_elevation_data,'Elevation',time_length,year_list, month_list, day_list)
final_static_population = Load_Static_Data.extract_population(static_population_data,'Population',time_length,year_list, month_list, day_list)
final_static_land_use_cover= Load_Static_Data.extract_land_use_cover(static_land_use_cover_data,'luc',time_length,year_list, month_list, day_list)
print('finish part 4')
