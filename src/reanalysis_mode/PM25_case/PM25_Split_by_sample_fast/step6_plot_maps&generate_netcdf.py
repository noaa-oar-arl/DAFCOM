...
'''
loaded_rf = joblib.load('./random_forest_June2023_model4.joblib')
'''
2) prepare input data 14
'''
from step1_Daily_Avr_InputMatrix import BLH, D2M, E, SP, T2M, TP, U10, V10, LAT, LON
from step1_Daily_Avr_InputMatrix import AOD, NDVI, Land_Use_Cover, Elevation, Population
from step1_Daily_Avr_InputMatrix import CMAQ_pm25_daily, E_BC_daily, E_SO2_daily, E_NOX_daily, E_VOC_daily, E_NH3_daily, E_PM25_daily
...
def GetPredDay(day_index):  #this is per day prediction
    ...
    input_test = np.reshape(np.transpose([V1_long,V2_long,V3_long,V4_long,V5_long,V6_long,V7_long,V8_long,V9_long,V10_long,V11_long,V12_long,V13_long,V14_long,V18_long,V19_long,V20_long,V21_long,V22_long,V23_long,V25_long,V26_long,V27_long,V28_long,V29_long,V30_long,V31_long,V32_long]),(14400000,28))
...
ft = Dataset(('CONUS_US_surfacePM25_0p01_regird'+'.nc'),'w',format = 'NETCDF4')   
...
variable_new = ft.createVariable('Surface pm25','f8',('time','latitude','longitude'))  
variable_new .units =''                                               
...
plt.savefig('ML_pm25_CONUS.png', dpi = 600)
'''
4-2) California
'''
...
plt.savefig('ML_pm25_California.png', dpi = 600)
...
# plt.title('1km*1km resolution Monthly average $PM_{2.5}$ concentraion \n in May-2021 \nmethod4 CAMS input, GEMS AOD, add spatial&time input')
plt.savefig('ML_pm25_LA.png', dpi = 300)
