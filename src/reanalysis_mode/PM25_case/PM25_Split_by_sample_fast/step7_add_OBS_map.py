...
EPA_PM25,LOC_NUMBER,LAT, LON,= GetData('CONUS_US_interpolated_June2023_fast')
...
'''
dir_1 = '/Volumes/Ext_Disk_1/3_NOAA_projects/NOAA_UFS_downscale/data/0_meteo/step2_fast/'
...
dir_2 = '/Volumes/Ext_Disk_1/3_NOAA_projects/NOAA_UFS_downscale/code/PM25_split_by_sample_MCMAQ_EGMU_AOD/'
pattern_1 = 'CONUS_US_surfacePM25_0p01_regird.nc'
filename_1 = dir_2 + pattern_1
f1 = Dataset(filename_1,'r')
# print(list(f1.variables))
Y_hat_month_daily = f1.variables['Surface pm25'][:] #(30,2500,3500)
...
plt.savefig('ML&OBS_pm25__CONUS.png', dpi = 600)
...
m.pcolor(LON, LAT, Y_hat_month,vmin = 0, vmax = 30,cmap = 'jet') 
...
#plot airkorea obs scatter maps
plt.scatter(np.array(Lon_bystation),np.array(Lat_bystation), c = np.array(Y_diff_bystation),cmap = 'jet',edgecolors='w',linewidth=3,s = 100)
cb = plt.colorbar()
plt.clim(0,30)
...
dir_shapefile='/Volumes/Ext_Disk_2/Data/shapefiles/USA/USA_County/'
m.readshapefile(dir_shapefile+"tl_rd22_us_county",'US_states')     
plt.savefig('ML&OBS_pm25_California.png', dpi = 600)
...
fig = plt.subplots(1,1,figsize = (15,10)) 
m.pcolor(LON, LAT, Y_hat_month,vmin = 0, vmax = 20,cmap = 'jet') 
...
plt.savefig('ML&OBS_pm25_LA.png', dpi = 600)
...
plt.savefig('ML&OBS_pm25__Denver.png', dpi = 600)
...
fig = plt.subplots(1,1,figsize = (15,10)) 
m.pcolor(LON, LAT, Y_hat_month,vmin = 0, vmax = 50,cmap = 'jet') 
...
plt.savefig('ML&OBS_pm25_BayArea.png', dpi = 600)
...
m.pcolor(LON, LAT, Y_hat_month,vmin =15 , vmax = 30,cmap = 'jet') 
                                
#plot airkorea obs scatter maps
plt.scatter(np.array(Lon_bystation),np.array(Lat_bystation), c = np.array(Y_diff_bystation),cmap = 'jet',edgecolors='k',linewidth=3,s = 200)
cb = plt.colorbar()
plt.clim(15,30)
cb.set_label('PM$_{2.5}$ (\u03bcg/$m^3$)',fontsize = 30) 
cb.ax.tick_params(labelsize=30)  
...
plt.savefig('ML&OBS_pm25_NYC.png', dpi = 600)
...
plt.savefig('ML&OBS_pm25_DC.png', dpi = 600)
...
fig = plt.subplots(1,1,figsize = (18,10)) 
m.pcolor(LON, LAT, Y_hat_month,vmin =13 , vmax = 17,cmap = 'jet') 
...
plt.savefig('ML&OBS_pm25_atlanta.png', dpi = 600)

'''
4-4) Tucson, AZ
'''
# #plot monthly averge map 
m = Basemap(projection='cyl', resolution='i',llcrnrlat=32.00, urcrnrlat = 32.35,llcrnrlon=-111.06, urcrnrlon = -110.70) # Aucson, AZ
fig = plt.subplots(1,1,figsize = (15,10)) 
m.pcolor(LON, LAT, Y_hat_month,vmin = 0, vmax = 10,cmap = 'jet') 
...
#add shape file
dir_shapefile='/Volumes/Ext_Disk_2/Data/shapefiles/USA/Tuscon_AZ/'
m.readshapefile(dir_shapefile+"speed_limit",'speed limit')  


    
plt.savefig('ML&OBS_pm25_Tuscon_AZ.png', dpi = 600)
...
plt.savefig('ML&OBS_pm25_Alberquque_NM.png', dpi = 600)
...
'''
# #plot monthly averge map 
m = Basemap(projection='cyl', resolution='h',llcrnrlat=41.77, urcrnrlat = 41.97,llcrnrlon=-87.72, urcrnrlon = -87.52) 
fig = plt.subplots(1,1,figsize = (18,10)) 
m.pcolor(LON, LAT, Y_hat_month,vmin =15 , vmax = 30,cmap = 'jet') 
...
# dir_shapefile='/Volumes/Ext_Disk_2/Data/shapefiles/USA/USA_County/'
# m.readshapefile(dir_shapefile+"tl_rd22_us_county",'US_states')    
dir_shapefile='/Volumes/Ext_Disk_2/Data/shapefiles/USA/4_Chicago_road/'
m.readshapefile(dir_shapefile+"test",'chicago_road',linewidth=0.1)
plt.savefig('ML&OBS_pm25_Chicago.png', dpi = 600)
