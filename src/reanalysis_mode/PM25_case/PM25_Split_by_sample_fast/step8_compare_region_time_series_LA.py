...
def GetData(ll_lat,ur_lat,ll_lon,ur_lon):
    file_loc = '/Volumes/Ext_Disk_1/3_NOAA_projects/NOAA_UFS_downscale/code/PM25_split_by_sample_MCMAQ_EGMU_AOD/'
    filename = 'CONUS_US_interpolated_June2023_fast'
    obs_df = pd.read_excel(file_loc+filename+'.xls')
    pd.set_option('display.max_columns',None)  
        
    LOC_NUMBER = list(obs_df['Location_number'])
    EPA_PM25 = list(obs_df['EPA_OBS_PM25'])
    UFS_PM25 = list(obs_df['V14_CMAQ_PM25'])
    LAT = list(obs_df['LAT'])
    LON = list(obs_df['LON'])
    ...
    for i in range(len(LOC_NUMBER)):
        if (LAT[i] - ll_lat)*(LAT[i] - ur_lat) <0:
            if (LON[i]-ll_lon)*(LON[i]-ur_lon) <0:
                ...
                UFS_PM25_domain.append(UFS_PM25[i])
                LAT_domain.append(LAT[i])
                LON_domain.append(LON[i])
                
                time_here_old = int(np.around(TIME[i],0))
                time_here_new = dt.strptime('23'+str(time_here_old),'%y%j').date()
                TIME_domain.append(time_here_new)
         
    return  EPA_pm25_domain,UFS_PM25_domain, LOC_number_domain,LAT_domain, LON_domain,TIME_domain


EPA_PM25_domain,UFS_PM25_domain, LOC_NUMBER_domain,LAT_domain, LON_domain,TIME_domain= GetData(33.7,34.2,-118.6,-117.8) #this is LA
...
for i in range(len(Location_number_bystation )):
    ...
    for j in range(len(LOC_NUMBER_domain)):
        if LOC_NUMBER_domain[j] == Location_number_bystation[i]:
            ...
            UFS_PM25_formal_this_site.append(UFS_PM25_domain[j])
            Time_formal_this_site.append(TIME_domain[j])
            
    EPA_PM25_formal.append(EPA_PM25_formal_this_site)
    UFS_PM25_formal.append(UFS_PM25_formal_this_site)
    Time_formal.append(Time_formal_this_site)
    
        
PM25_OBS_site1 = EPA_PM25_formal[0][0:30]
...
UFS_OBS_site1 = UFS_PM25_formal[0][0:30]
UFS_OBS_site2 = UFS_PM25_formal[1]
UFS_OBS_site3 = UFS_PM25_formal[2]
UFS_OBS_site4 = UFS_PM25_formal[3]
UFS_OBS_site5 = UFS_PM25_formal[4]
UFS_OBS_site6 = UFS_PM25_formal[5]
UFS_OBS_site7 = UFS_PM25_formal[6]
UFS_OBS_site8 = UFS_PM25_formal[7]


TIME_OBS_site1 = Time_formal[0][0:30]
...
dir_ml = '/Volumes/Ext_Disk_1/3_NOAA_projects/NOAA_UFS_downscale/code/PM25_split_by_sample_MCMAQ_EGMU_AOD/'
...
base_time_ml = dt(2023,6,1,0,0,0)
...
plt.plot(TIME_OBS_site1,UFS_OBS_site1,'b-',label = 'UFS AQM')
plt.plot(TIME_ML_all_site,PM25_ML_site1,'r-',label = 'ML prediction')
plt.tick_params(labelbottom = False, bottom = False)
plt.xlim(dt(2023,6,1,0,0,0),dt(2023,7,1,0,0,0))
plt.ylim(0,20)
plt.ylabel('$PM_{2.5}$ \n (\u03bcg/$m^3$)',fontsize = 20)
...
plt.plot(TIME_OBS_site2,UFS_OBS_site2,'b-',label = 'UFS AQM')
...
plt.plot(TIME_OBS_site3,UFS_OBS_site3,'b-',label = 'UFS AQM')
...
plt.plot(TIME_OBS_site4,UFS_OBS_site4,'b-',label = 'UFS AQM')
...
plt.plot(TIME_OBS_site5,UFS_OBS_site5,'b-',label = 'UFS AQM')
...
plt.ylabel('$PM_{2.5}$ \n (\u03bcg/$m^3$)',fontsize = 20)
plt.legend(loc = 'upper right',fontsize = 20)
plt.savefig('LA_compare_1stHalf5sites_withAOD',dpi = 600)
...
# ax2.xaxis.set_major_formatter(myFmt)
# plt.plot(TIME_OBS_site7,PM25_OBS_site7,'k*',label = 'EPA OBS @ site '+str(Location_number_bystation[6]))
...
# ax3.xaxis.set_major_formatter(myFmt)
# plt.plot(TIME_OBS_site8,PM25_OBS_site8,'k*',label = 'EPA OBS @ site '+str(Location_number_bystation[7]))
...
# ax4.xaxis.set_major_formatter(myFmt)
# plt.plot(TIME_OBS_site9,PM25_OBS_site9,'k*',label = 'EPA OBS @ site '+str(Location_number_bystation[8]))
...
# ax5.xaxis.set_major_formatter(myFmt)
# plt.plot(TIME_OBS_site10,PM25_OBS_site10,'k*',label = 'EPA OBS @ site '+str(Location_number_bystation[9]))
...
# plt.plot(TIME_OBS_site11,PM25_OBS_site11,'k*',label = 'EPA OBS @ site '+str(Location_number_bystation[10]))
...
# plt.savefig('LA_compare_2ndHalf6sites_withAOD',dpi = 600)