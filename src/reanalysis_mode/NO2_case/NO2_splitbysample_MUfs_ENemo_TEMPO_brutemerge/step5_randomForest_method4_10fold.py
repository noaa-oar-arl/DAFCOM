...
def GetData(filename):
    ...
    EPA_NO2 = list(obs_df['EPA_OBS_NO2'])
    ...
    V14_CMAQ_NO2 = list(obs_df['V14_CMAQ_NO2'])
    V15_MERGE_NO2 = list(obs_df['V15_MERGE_NO2'])
    V16_TROPOMI_NO2 = list(obs_df['V16_TROPOMI_NO2'])
    ...
    return  V1_BLH, V2_D2M,V3_E, V4_SP, V5_T2M, V6_TP, V7_U10, V8_V10,V9_AOD,V10_NDVI, V11_LAND_USE_COVER,V12_ELEVATION,V13_POPULATION, V14_CMAQ_NO2,V15_MERGE_NO2,V16_TROPOMI_NO2,V18_E_BC, V19_E_SO2, V20_E_NOX, V21_E_VOC, V22_E_NH3, V23_E_PM25, EPA_NO2,LOC_NUMBER,LAT, LON, D1, D2, D3, D4, D5,JULIAN_DAY


V1_BLH, V2_D2M,V3_E, V4_SP, V5_T2M, V6_TP, V7_U10, V8_V10, V9_AOD, V10_NDVI, V11_LAND_USE_COVER,V12_ELEVATION,V13_POPULATION, V14_CMAQ_NO2,V15_MERGE_NO2,V16_TROPOMI_NO2,V18_E_BC, V19_E_SO2, V20_E_NOX, V21_E_VOC, V22_E_NH3, V23_E_PM25, EPA_NO2,LOC_NUMBER,LAT, LON, D1, D2, D3, D4, D5,JULIAN_DAY = GetData('CONUS_US_interpolated_Aug2023_NO2')
...
def InputAndOutput(Index_part1_input):
    ...
    for i in range(len(Y)):
        if i in Index_part1_input:
            ...
            X15_test.append(X15[i])
            # X16_test.append(X16[i])
            ...
            X15_train.append(X15[i])
            # X16_train.append(X16[i])
    ...
    input_train = np.reshape(np.transpose([X1_train,X2_train,X3_train,X4_train,X5_train,X6_train,X7_train,X8_train,X9_train,X10_train,X11_train,X12_train,X13_train,X14_train,X15_train,X18_train,X19_train,X20_train,X21_train,X22_train,X23_train,X25_train,X26_train,X27_train,X28_train,X29_train,X30_train,X31_train,X32_train]),(len_train,29))
    output_train = Y_train
    input_test = np.reshape(np.transpose([X1_test,X2_test,X3_test,X4_test,X5_test,X6_test,X7_test,X8_test, X9_test,X10_test,X11_test,X12_test,X13_test,X14_test,X15_test,X18_test,X19_test,X20_test,X21_test,X22_test,X23_test,X25_test,X26_test,X27_test,X28_test,X29_test,X30_test,X31_test,X32_test]),(len_test,29))
    output_test = Y_test
...
feature_list = ['V1_BLH','V2_D2M','V3_E','V4_SP','V5_T2M','V6_TP','V7_U10','V8_V10','V9_AOD','V10_NDVI','V11_LAND_USE_COVER','V12_ELEVATION','V13_POPULATION',
                'V14_CMAQ_NO2','V15_MERGE_NO2','V18_E_BC','V19_E_SO2','V20_E_NOX','V21_E_VOC','V22_E_NH3','V23_E_PM25','LAT','LON','D1','D2','D3','D4','D5','Julian_day(time)']
