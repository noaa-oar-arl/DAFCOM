...
EPA_PM25_domain,UFS_PM25_domain,LOC_NUMBER_domain,LAT_domain, LON_domain,TIME_domain= GetData(39.65,39.80,-105.05,-104.9) #this is Denver
...
UFS_OBS_site1 = UFS_PM25_formal[0]
...
dir_ml = '/Volumes/Ext_Disk_1/3_NOAA_projects/NOAA_UFS_downscale/NO2_case/code/NO2_splitbysample_MUfs_ENemo_TEMPO/'
        ...
        if (lat_cams[i-1]-lat_obs)*(lat_cams[i]-lat_obs) < 0: 
            ...
            for j in range(1,6000):   #400         
                if (lon_cams[j-1]-lon_obs)*(lon_cams[j]-lon_obs) < 0:     
                    ...
                    for k in range(31):  #2018-Nov HAVE 30 DAYS
...
base_time_ml = dt(2023,8,1,0,0,0)
TIME_ML_all_site = [base_time_ml + datetime.timedelta(days=x) for x in range(31)]
'''
...
plt.xlim(dt(2023,8,1,0,0,0),dt(2023,9,1,0,0,0))
plt.ylim(0,20)
...
plt.savefig('Denver_compare_5sites.png',dpi = 600)
