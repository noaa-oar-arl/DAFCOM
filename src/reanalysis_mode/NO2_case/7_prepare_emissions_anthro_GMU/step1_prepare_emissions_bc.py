...
'''
dir_1  = '/Volumes/Ext_Disk_2/Data/emission/Siqi_GMU_1km_anthro/2019/'
pattern_1= 'emis_mole_all_201906_US01_cmaq_cb6ae7_2019ge_19j_mean.ncf'  
filename_1 = dir_1+pattern_1
f1 = Dataset(filename_1,'r')   

V_list1 = list(f1.variables)   
V1 = f1.variables[name_variable][:][0][0] #(3177,5397)
...
'''
2) check if we have some value <0
'''
print(np.min(V1))
...
Emission_Korea = V1*3600*24/(1000)  #convert unit from g/s to kg/day


'''
3) slip lat directions
'''
# LAT_list = np.flip(LAT)
LAT_list = LAT
LON_list = LON
# Emission_Korea = np.flip(Emission_Korea,axis =0 )
...
4)write in netcdf
'''
ft = Dataset('Emission_BC_step1.nc','w',format = 'NETCDF4')                    #Change2
...
variable_new = ft.createVariable('E_bc','f4',('time','latitude','longitude'))  #Change3
variable_new .units =''                                               
