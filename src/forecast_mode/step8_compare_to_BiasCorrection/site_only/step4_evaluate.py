...
bc_pm25 = list(df['bias_correction_pm25'])
...
std_model = np.std(X4)
std_obs = np.std(X1)
print('this is Bias Correction Team Result vs. OBS')
print('r test',r_value_test)
print('mse test',mse_test)
print('mean bias test', mean_bias)
print('std model =',std_model)
...
slope_test, intercept_test, r_value_test, p_value_test, std_err_test = scipy.stats.linregress(np.array(X1), np.array(X5))
mse_test = mean_squared_error(X1, X5)
mean_bias= np.mean(np.array(X5)-np.array(X1))
std_model = np.std(X5)
std_obs = np.std(X1)
