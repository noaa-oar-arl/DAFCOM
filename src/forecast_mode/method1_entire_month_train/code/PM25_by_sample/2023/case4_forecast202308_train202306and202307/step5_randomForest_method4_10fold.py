...
def GetData(filename):
    file_loc = '/groups/ESS/btang6/Projects/NOAA/Forecast_UFS/method1_entire_month_train/code/PM25_by_sample/2023/case4_forecast202308_train202306and202307/'
    obs_df = pd.read_excel(file_loc+filename)
...
V1_BLH, V2_D2M,V3_E, V4_SP, V5_T2M, V6_TP, V7_U10, V8_V10,V9_AOD, V11_LAND_USE_COVER,V12_ELEVATION,V13_POPULATION, V14_UFS_AQM_PM25,V18_E_BC, V19_E_SO2, V20_E_NOX, V21_E_VOC, V22_E_NH3, V23_E_PM25,EPA_OBS_PM25,LOC_NUMBER,LAT, LON, D1, D2, D3, D4, D5,JULIAN_DAY = GetData('CONUS_US_interpolated_2023_06and07_PM25.xls')
...
def InputAndOutput(Index_part1_input):
    ...
    input_train = np.reshape(np.transpose([X1_train,X2_train,X3_train,X4_train,X5_train,X6_train,X7_train,X8_train,X9_train,X11_train,X12_train,X13_train,X14_train,X18_train,X19_train,X20_train,X21_train,X22_train,X23_train,X25_train,X26_train,X27_train,X28_train,X29_train,X30_train,X31_train,X32_train]),(len_train,27))
    output_train = Y_train
    input_test = np.reshape(np.transpose([X1_test,X2_test,X3_test,X4_test,X5_test,X6_test,X7_test,X8_test, X9_test,X11_test,X12_test,X13_test,X14_test,X18_test,X19_test,X20_test,X21_test,X22_test,X23_test,X25_test,X26_test,X27_test,X28_test,X29_test,X30_test,X31_test,X32_test]),(len_test,27))
    output_test = Y_test
...
import joblib
joblib.dump(rf_best_1,'./random_forest_model1.joblib')
joblib.dump(rf_best_2,'./random_forest_model2.joblib')
joblib.dump(rf_best_3,'./random_forest_model3.joblib')
joblib.dump(rf_best_4,'./random_forest_model4.joblib')
joblib.dump(rf_best_5,'./random_forest_model5.joblib')
joblib.dump(rf_best_6,'./random_forest_model6.joblib')
joblib.dump(rf_best_7,'./random_forest_model7.joblib')
joblib.dump(rf_best_8,'./random_forest_model8.joblib')
joblib.dump(rf_best_9,'./random_forest_model9.joblib')
joblib.dump(rf_best_10,'./random_forest_model10.joblib')
...
feature_list = ['V1_BLH','V2_D2M','V3_E','V4_SP','V5_T2M','V6_TP','V7_U10','V8_V10','V9_AOD','V11_LAND_USE_COVER','V12_ELEVATION','V13_POPULATION',
                'V14_UFS_AQM_PM25','V18_E_BC','V19_E_SO2','V20_E_NOX','V21_E_VOC','V22_E_NH3','V23_E_PM25','LAT','LON','D1','D2','D3','D4','D5','Julian_day(time)']
